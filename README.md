...
+ Ver 1.3.2: JSON 파싱 오류 발생 시 빈 JSON 파일 재생성하게 수정, 타임아웃 시 로그를 남긴 후 exit(1)을 호출하게 수정, 위젯이 mount된 경우에만 상태 업데이트를 수행하도록 변경, 각 초기화 작업을 명확히 분리, 분산되어 있던 여러 setState 호출 및 비동기 작업들을 통합·분리
